...
select * from produtos where categoria != 'Bebidas' order by categoria;

/* 3. Produtos descontinuados. */
select nome, categoria, descontinuado, unidades_estoque from produtos where descontinuado = 'Sim' order by categoria;

/* 4. Produtos com unidades em estoque igual a 0. */
select * from produtos where unidades_estoque = 0;
...
select * from produtos where categoria in ('Bebidas', 'Condimentos', 'Confeitos') order by categoria;

/* 8. Produtos da categoria Bebidas com mais de 100 unidades em estoque. */
select nome, categoria, unidades_estoque from produtos where categoria = 'Bebidas' and unidades_estoque > 100 order by unidades_estoque;

/* 9. Produtos da categoria Bebidas ou Laticínios com mais de 100 unidades em estoque. */
select nome, categoria, unidades_estoque from produtos where categoria in ('Bebidas', 'Laticínios') and unidades_estoque > 100 order by unidades_estoque; 

/* 10. Produtos descontinuados com unidades em estoque diferente de 0. */
select nome, categoria, unidades_estoque, descontinuado from produtos where descontinuado = 'Sim' and unidades_estoque != 0 order by categoria;

/* 11. Produtos iniciando com a letra Q. */
select nome, categoria from produtos where nome like 'q%';
...
select count(*) from produtos where categoria = 'Confeitos' and preco_unitario > 15.0;

/* 4. Quantidade de produtos da categoria Confeitos com preço maior que 15,0 e unidades em estoque maior que 50. */
select count(*) from produtos where categoria = 'Confeitos' and preco_unitario > 15.0 and unidades_estoque > 50;

/* 5. Maior preço da categoria Bebidas. */
select max(preco_unitario) from produtos where categoria = 'Bebidas';

/* 6. Menor preço da categoria Bebidas. */
select min(preco_unitario) from produtos where categoria = 'Bebidas';

/* 7. Preço médio dos produtos da categoria Bebidas. */
select avg(preco_unitario) from produtos where categoria = 'Bebidas';

/* 8. Total de unidades em estoque da categoria Confeitos. */
select sum(unidades_estoque) from produtos where categoria = 'Confeitos';

/* 9. Número de produtos descontinuados. */
select sum(descontinuado = 'Sim') from produtos;

/* 10. Preço dos produtos da categoria Bebidas com 10% de aumento. */
select nome, categoria, preco_unitario * 1.1 as preco_aumento from produtos where categoria = 'Bebidas';

/* 11. Preço dos produtos da categoria Laticínios com 5% de desconto. */
select nome, categoria, preco_unitario * 0.95 as preco_desconto from produtos where categoria = 'Laticínios';

/* 12. Preço dos produtos com preço unitário acima de 40,0 com 7% de desconto. */
select nome, preco_unitario * 0.93 as preco_desconto from produtos where preco_unitario > 40.0;
...
select categoria, count(*) as descontinuados from produtos where descontinuado = 'Sim' group by categoria;

/* 3. Categorias com mais de um produto descontinuado. */
select categoria, count(*) as descontinuados from produtos where descontinuado = 'Sim' group by categoria having count(*) > 1; 
...
select categoria, count(*) as quantidade from produtos group by categoria having count(*) <= 5;

/* 6. Categorias com mais de 10 produtos. */
select categoria, count(*) as quantidade from produtos group by categoria having count(*) > 10;
